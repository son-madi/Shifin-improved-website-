...
export const AdminPage: React.FC<AdminPageProps> = ({
  ...
  const handleDeleteAccount = async (targetUserId: string, username: string) => {
    ...
    setError(null);
    try {
      const res = await authFetch(`/api/admin/accounts/${targetUserId}`, {
        method: 'DELETE',
      });
      const data = await res.json();
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-800/60 pb-3">
                        <div className="flex items-center gap-3.5">
                          <div className={`w-11 h-11 rounded-2xl border flex items-center justify-center font-black text-sm shadow-md shrink-0 ${
                            account.isAdmin
