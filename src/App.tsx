...
export default function App() {
  ...
  const dockTabConfig: Record<string, {
    ...
    hud_chat: {
      activeText: !isColourUI ? (isDark ? 'text-white font-extrabold' : 'text-slate-900 font-extrabold') : 'text-emerald-300 font-extrabold',
      iconColor: !isColourUI ? (isDark ? 'text-white' : 'text-slate-900') : 'text-emerald-400',
      pillClass: isDark
        ? isColourUI
          ? 'bg-emerald-950/60 border-emerald-500/70 text-emerald-200 shadow-[0_0_16px_rgba(16,185,129,0.35)] ring-1 ring-emerald-400/50'
        ...
        : 'bg-emerald-50 border-emerald-500 text-emerald-900 shadow-[0_0_12px_rgba(16,185,129,0.2)] ring-1 ring-emerald-300',
      containerBorder: !isColourUI ? (isDark ? 'border-zinc-700/80 shadow-[0_0_20px_rgba(255,255,255,0.03)]' : 'border-slate-300 shadow-[0_0_20px_rgba(0,0,0,0.04)]') : 'border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.06)]',
    },
    anti_afk: {
              ...
              <div className="grid grid-cols-3 sm:flex sm:items-center gap-1 sm:gap-1.5 w-full sm:w-auto flex-1 py-0.5">
                <motion.button
                  ...
                  className={`w-full sm:w-auto flex-1 sm:flex-initial flex items-center justify-center gap-1 sm:gap-1.5 py-2 px-1.5 sm:px-3 rounded-xl font-bold text-[11px] sm:text-xs transition-colors duration-200 relative z-10 cursor-pointer whitespace-nowrap overflow-hidden border text-center ${
                    activeTab === 'hud_chat'
                      ...
                      : isColourUI
                      ? 'border-slate-800/80 hover:border-emerald-500/50 bg-slate-900/40 hover:bg-slate-800/60 text-slate-400 hover:text-slate-200'
                      : isDark
                ...
                >
                  <Terminal className={`w-3.5 h-3.5 shrink-0 transition-colors duration-200 ${
                    isColourUI
                      ? (activeTab === 'hud_chat' ? 'text-emerald-400' : 'text-emerald-500/70')
                      : activeTab === 'hud_chat'
